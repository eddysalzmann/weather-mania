<template>
	...
	<section class="search">
		<div class="search-inner">
			...
			/>
			<div v-if="errors" class="error flag">
				{{errors}}
			</div>
...
export default {
  ...
  watch: {
	...
	methods: {
		addPlace () {
			var exists = false
			const trimmedText = this.newPlace.trim()
			if (trimmedText) {
				let API_KEY = "9f2a6791591faa35ae205c04b592baef";
				if (localStorage.getItem('appID')) {
          API_KEY = JSON.parse(localStorage.getItem('appId'));
        }
				let CURRENT_WEATHER = `http://api.openweathermap.org/data/2.5/weather?q=${trimmedText}&units=metric&appid=${API_KEY}`;
				axios
					.get(CURRENT_WEATHER)
					.then(response => {
